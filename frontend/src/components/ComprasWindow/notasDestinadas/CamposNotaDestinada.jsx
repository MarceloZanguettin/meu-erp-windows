...
import { GRUPOS_CAMPOS_NOTA_DESTINADA } from './camposNotaDestinada.js';
...
/**
 * Renderiza todos os campos de GENUS.NOTASDESTINADAS agrupados em fieldsets,
 * a partir da config declarativa em camposNotaDestinada.js. Usado tanto pela
 * janela de listagem/edição (NotaDestinadaWindow) quanto pela janela de
 * criação (NovaNotaDestinadaWindow) — mesmo padrão já usado em
 * ComprasWindow/entrada/CamposEntrada.jsx.
 */
export default function CamposNotaDestinada({ form, setForm }) {
  const setField = (campo, valor) => setForm({ ...form, [campo]: valor });

  return (
    <>
      {GRUPOS_CAMPOS_NOTA_DESTINADA.map(grupo => (
