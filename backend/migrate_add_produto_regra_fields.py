"""
Migração aditiva: cria/expande a tabela `produto_regras` (modelo
ProdutoRegra — campos migrados da tabela PRODUTOREGRAS do GENUS), sem apagar
nada.
...
Execute com: python migrate_add_produto_regra_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ProdutoRegra.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ProdutoRegra.__table__])
...
with engine.begin() as conn:
    for coluna in ProdutoRegra.__table__.columns:
