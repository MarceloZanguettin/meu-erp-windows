"""
Migração aditiva: cria/expande a tabela `itens_compra` (modelo ItemCompra —
campos migrados da tabela COMPRASLAN do GENUS), sem apagar nada.
...
Execute com: python migrate_add_item_compra_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ItemCompra.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ItemCompra.__table__])
...
with engine.begin() as conn:
    for coluna in ItemCompra.__table__.columns:
