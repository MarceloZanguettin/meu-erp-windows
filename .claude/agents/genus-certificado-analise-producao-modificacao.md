---
name: genus-certificado-analise-producao-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `CERTIFICADO_ANALISE_PRODUCAO` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela CERTIFICADO_ANALISE_PRODUCAO", "adapta o backend pra reconhecer os campos de CERTIFICADO_ANALISE_PRODUCAO", "migra a estrutura de CERTIFICADO_ANALISE_PRODUCAO do GENUS".
...
`CERTIFICADO_ANALISE_PRODUCAO`, following the exact precedent already established for
...
## Fields to recognize (GENUS table `CERTIFICADO_ANALISE_PRODUCAO`)
...
| HORA | String(8) |
| FABRICACAO | DateTime |
| NOTAFISCAL | String(50) |
| LOTE | String(50) |
| VALIDADE | DateTime |
| QUANTIDADE_TOTAL | Float |
| CLINOME | String(45) |
| CLIENDERECO | String(50) |
| CLINUMERO | String(6) |
| CLIBAIRRO | String(35) |
| CLICODCIDADE | Integer |
| CLICEP | String(10) |
| CLICONTATO | String(40) |
| CLIFONE | String(15) |
| CODCLIENTE | Integer |
| CODEMPRESAPRODETAPAS | Integer |
| CODPRODETAPAS | Integer |
| LOTEPRODETAPAS | String(10) |
| PESO | Float |
| CODOPERADOR | Integer |
| PRODUCAOFINALIZADA | String(1) |
| ESPESSURA | Float |
| LARGURA | Float |
| PESOTARUGO | Float |
| PESOAMOSTRA | Float |
