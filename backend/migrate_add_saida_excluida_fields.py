"""
Migração aditiva: cria/expande a tabela `saidas_excluidas` (modelo
SaidaExcluida — campos migrados da tabela DELSAIDA do GENUS), sem apagar
nada.
...
Execute com: python migrate_add_saida_excluida_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = SaidaExcluida.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[SaidaExcluida.__table__])
...
with engine.begin() as conn:
    for coluna in SaidaExcluida.__table__.columns:
