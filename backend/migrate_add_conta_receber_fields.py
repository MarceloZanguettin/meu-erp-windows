"""
Migração aditiva: adiciona à tabela `contas_receber` as colunas novas do
modelo ContaReceber (campos migrados da tabela RECEBER do GENUS), sem
apagar nada.
...
Execute com: python migrate_add_conta_receber_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
from models.tabelas import ContaReceber

inspector = inspect(engine)
tabela = ContaReceber.__tablename__
...
# Contagem de linhas antes da migração (apenas leitura, nenhum dado é alterado)
with engine.connect() as conn:
    linhas_antes = conn.execute(text(f'SELECT COUNT(*) FROM {tabela}')).scalar()

colunas_existentes = {c["name"] for c in inspector.get_columns(tabela)}

adicionadas = []
with engine.begin() as conn:
    for coluna in ContaReceber.__table__.columns:
...
# Contagem de linhas depois da migração, para confirmar que nada foi perdido
with engine.connect() as conn:
    linhas_depois = conn.execute(text(f'SELECT COUNT(*) FROM {tabela}')).scalar()

print(f"Linhas em '{tabela}' antes: {linhas_antes} | depois: {linhas_depois}")
if linhas_antes != linhas_depois:
    print("⚠️  ATENÇÃO: a contagem de linhas mudou — investigue antes de prosseguir!")
else:
    print("Nenhum dado existente em contas_receber (ou em qualquer outra tabela) foi apagado ou alterado.")
