"""
Migração aditiva: cria/expande a tabela `itens_saida` (modelo ItemSaida —
campos migrados da tabela SAILAN do GENUS), sem apagar nada.
...
Execute com: python migrate_add_item_saida_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ItemSaida.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ItemSaida.__table__])
...
with engine.begin() as conn:
    for coluna in ItemSaida.__table__.columns:
