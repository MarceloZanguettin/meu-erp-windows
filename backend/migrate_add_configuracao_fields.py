"""
Migração aditiva: cria/expande a tabela `configuracoes` com as colunas do
modelo Configuracao (campos migrados da tabela CONFIGURACAO do GENUS), sem
apagar nada.
...
1. Se a tabela `configuracoes` ainda não existir, cria SOMENTE essa tabela via
...
Execute com: python migrate_add_configuracao_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = Configuracao.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[Configuracao.__table__])
...
print("Nenhum dado existente em configuracoes (ou em qualquer outra tabela) foi apagado ou alterado.")
