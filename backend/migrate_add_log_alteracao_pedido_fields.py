"""
Migração aditiva: cria/expande a tabela `log_alteracoes_pedido` (modelo
LogAlteracaoPedido — campos migrados da tabela LOGALTERACAOPEDIDO do
...
Execute com: python migrate_add_log_alteracao_pedido_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = LogAlteracaoPedido.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[LogAlteracaoPedido.__table__])
...
with engine.begin() as conn:
    for coluna in LogAlteracaoPedido.__table__.columns:
