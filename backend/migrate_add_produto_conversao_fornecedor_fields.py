"""
Migração aditiva: cria/expande a tabela `produto_conversoes_fornecedor`
(modelo ProdutoConversaoFornecedor — campos migrados da tabela
PRODUTOCONVERSAOFORNECEDOR do GENUS), sem apagar nada.
...
Execute com: python migrate_add_produto_conversao_fornecedor_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ProdutoConversaoFornecedor.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ProdutoConversaoFornecedor.__table__])
...
with engine.begin() as conn:
    for coluna in ProdutoConversaoFornecedor.__table__.columns:
