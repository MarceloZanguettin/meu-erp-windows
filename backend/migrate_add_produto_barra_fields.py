"""
Migração aditiva: cria/expande a tabela `produto_barras` (modelo
ProdutoBarra — campos migrados da tabela PRODUTOBARRA do GENUS), sem apagar
nada.
...
Execute com: python migrate_add_produto_barra_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ProdutoBarra.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ProdutoBarra.__table__])
...
with engine.begin() as conn:
    for coluna in ProdutoBarra.__table__.columns:
