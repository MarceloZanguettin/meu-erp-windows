"""
Migração aditiva: cria/expande a tabela `produto_producoes` (modelo
ProdutoProducao — campos migrados da tabela PRODUTOPRODUCAO do GENUS), sem
apagar nada.
...
Execute com: python migrate_add_produto_producao_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ProdutoProducao.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ProdutoProducao.__table__])
...
with engine.begin() as conn:
    for coluna in ProdutoProducao.__table__.columns:
