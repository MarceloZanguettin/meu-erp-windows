"""
Migração aditiva: cria/expande a tabela `produtos_excluidos` (modelo
ProdutoExcluido — campos migrados da tabela DEL_PRODUTO do GENUS), sem
apagar nada.
...
Execute com: python migrate_add_produto_excluido_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ProdutoExcluido.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ProdutoExcluido.__table__])
...
with engine.begin() as conn:
    for coluna in ProdutoExcluido.__table__.columns:
