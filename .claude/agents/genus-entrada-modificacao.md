---
name: genus-entrada-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `ENTRADA` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela ENTRADA", "adapta o backend pra reconhecer os campos de ENTRADA", "migra a estrutura de ENTRADA do GENUS".
...
`ENTRADA`, following the exact precedent already established for
...
## Fields to recognize (GENUS table `ENTRADA`)
...
| MODELO | String(2) |
| CODCONTROLE | Integer |
| CODCONTROLEEMPRESA | Integer |
| CODCONTROLETIPO | String(1) |
| CODTIPOCOMPRA | Integer |
| OUTROSCUSTO | Float |
| QUANTIDADE | String(10) |
| RETERIMPOSTO | String(1) |
| CODEMPRESANAOFISCAL | Integer |
| CODSAIDAVINCULADA | Integer |
| CODEMPRESASAIDAVINCULADA | Integer |
| DOCSAIDAVINCULADA | Integer |
| ALIQUOTA_SUBS | String(5) |
| ICMSREDUCAO | Float |
| VALORCREDITOFORNECEDOR | Float |
| CODEMPRESAPRODUCAO | Integer |
| CODIGOPRODUCAO | Integer |
| LOTEPRODUCAO | String(10) |
