"""
Migração aditiva: cria/expande a tabela `contas_pagar_excluidas` (modelo
ContaPagarExcluida — campos migrados da tabela DELPAGAR do GENUS), sem
apagar nada.
...
Execute com: python migrate_add_conta_pagar_excluida_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ContaPagarExcluida.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ContaPagarExcluida.__table__])
...
with engine.begin() as conn:
    for coluna in ContaPagarExcluida.__table__.columns:
