"""
Migração aditiva: cria/expande a tabela `itens_orcamento_genus` (modelo
ItemOrcamentoGenus — campos migrados da tabela ORCAMENTO2 do GENUS), sem
apagar nada.
...
Execute com: python migrate_add_item_orcamento_genus_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ItemOrcamentoGenus.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ItemOrcamentoGenus.__table__])
...
with engine.begin() as conn:
    for coluna in ItemOrcamentoGenus.__table__.columns:
