"""
Migração aditiva: cria/expande a tabela `precos_produto` (modelo
PrecoProduto — campos migrados da tabela PRECO do GENUS), sem apagar nada.
...
Execute com: python migrate_add_preco_produto_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = PrecoProduto.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[PrecoProduto.__table__])
...
with engine.begin() as conn:
    for coluna in PrecoProduto.__table__.columns:
