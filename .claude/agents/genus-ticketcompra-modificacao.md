---
name: genus-ticketcompra-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `TICKETCOMPRA` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela TICKETCOMPRA", "adapta o backend pra reconhecer os campos de TICKETCOMPRA", "migra a estrutura de TICKETCOMPRA do GENUS".
...
`TICKETCOMPRA`, following the exact precedent already established for
...
## Fields to recognize (GENUS table `TICKETCOMPRA`)
...
| PESONOTA | Float |
| DIFERENCANOTA | Float |
| PERCENTUALDIFERENCA | Float |
| DATAENTRADA | DateTime |
| HORAENTRADA | String(8) |
| DATASAIDA | DateTime |
| HORASAIDA | String(8) |
| TIPOVEICULO | String(1) |
| PLACA | String(8) |
| PLACACARRETA | String(8) |
| MOTORISTA | String(45) |
| CODFORNECEDOR | Integer |
| CODAGREGADO | Integer |
| TRANSPORTADORA | String(45) |
| CODNOTA | Integer |
| OBSERVACAO | Text |
| CODCOMPRA | Integer |
| CODEMPRESANOTA | Integer |
| TIPODOCNOTA | String(1) |
| SERIENOTA | String(4) |
| PRECOUNITARIO | Float |
| VENDEDOR | String(45) |
| VALORBRUTO | Float |
