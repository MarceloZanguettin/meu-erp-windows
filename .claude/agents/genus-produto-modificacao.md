---
name: genus-produto-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `PRODUTO` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela PRODUTO", "adapta o backend pra reconhecer os campos de PRODUTO", "migra a estrutura de PRODUTO do GENUS".
...
`PRODUTO`, following the exact precedent already established for
...
JÁ MIGRADO NESTA SESSÃO — model Produto, schema produto.py e controller produto_controller.py já reconhecem todos os campos desta tabela. Rodar este agente deve ser um no-op: apenas confirme que nada mudou no schema do GENUS.

## Fields to recognize (GENUS table `PRODUTO`)
...
| PESOLIQUIDO | Float |
| PESOBRUTO | Float |
| CODINTERNO | String(30) |
| TIPOPRODUTO | String(1) |
| VALIDADE | Integer |
| CUBICAGEM | Float |
| OBS | Text |
| DESCRIINTERNA | String(50) |
| REFERENCIA | String(20) |
| MULTIPLOPRODUCAO | Float |
| DESCRIDETALHADA | Text |
| CODCOR | Integer |
| CODTAMANHO | String(5) |
| CODALTERACAO | Integer |
| HORAALTERACAO | String(8) |
| DATAALTERACAO | DateTime |
| CODFUNCIONARIOINSERE | Integer |
| CODFUNCIONARIOALTERA | Integer |
| FATORCONVERSAO | Float |
| CODEVENTO | Integer |
| CODCONTABILPRAZO | String(4) |
| CODCONTABILAVISTA | String(4) |
| SEQCODBARRA | String(13) |
| SEGURO | DateTime |
...
| ANP | String(9) |
| KILOSRECEITA | Float |
| UNIDADECOMPRA | String(3) |
| TIPOCONVERSAO | String(1) |
| KGMT | Float |
| UNDE | Float |
| CODGRADE | Integer |
| TAMANHOPROD | Integer |
| CODPRODUTOGRADE | String(15) |
| CODLINHA | Integer |
| ALTURA | Float |
| COMPRIMENTO | Float |
| LARGURA | Float |
| METROSCUBICOS | Float |
| TIPOPRODUTOFABRICA | String(1) |
| PONTEIRATIPOBOX | String(1) |
| PONTEIRATIPODECOTE | String(1) |
| PONTEIRATIPO | String(1) |
| DATA_HORA_ALTERADO | DateTime |
| RELATORIOTABELAPRECO | String(1) |
| COD_EMPRESA_TRANSF | Integer |
| COD_ANTIGO_TRANSFERE1 | Integer |
| COD_ANTIGO_TRANSFERE2 | Integer |
| COD_EMPRESA_TRANSF1 | Integer |
| COD_EMPRESA_TRANSF2 | Integer |
| TIPO | String(1) |
| CODCFOPESTADO | String(5) |
| CODCFOPFORAESTADO | String(5) |
| CODSECUNDARIO | String(30) |
| ESPESSURA | Float |
