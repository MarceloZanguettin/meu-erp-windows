"""
Migração aditiva: cria/expande a tabela `agregados` com as colunas do modelo
Agregado (campos migrados da tabela AGREGADOS do GENUS), sem apagar nada.
...
1. Se a tabela `agregados` ainda não existir, cria SOMENTE essa tabela via
   `Base.metadata.create_all(bind=engine, tables=[...])` — o mesmo mecanismo
   não-destrutivo que `main.py` já usa no startup
...
Execute com: python migrate_add_agregado_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[Agregado.__table__])
...
with engine.begin() as conn:
    for coluna in Agregado.__table__.columns:
...
print("Nenhum dado existente em agregados (ou em qualquer outra tabela) foi apagado ou alterado.")
