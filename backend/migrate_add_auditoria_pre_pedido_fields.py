"""
Migração aditiva: cria/expande a tabela `auditorias_pre_pedido` (modelo
AuditoriaPrePedido — campos migrados da tabela AUDITORIA_PREPEDIDO do
...
Execute com: python migrate_add_auditoria_pre_pedido_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = AuditoriaPrePedido.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[AuditoriaPrePedido.__table__])
...
with engine.begin() as conn:
    for coluna in AuditoriaPrePedido.__table__.columns:
