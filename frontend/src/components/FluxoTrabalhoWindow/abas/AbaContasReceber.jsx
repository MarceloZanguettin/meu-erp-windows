...
export default function AbaContasReceber({ empresas }) {
  ...
  const carregar = () => {
    axios.get(`${API}/financeiro/contas-receber`)
  ...
  const salvar = async () => {
    ...
    try {
      if (editando) {
        await axios.put(`${API}/financeiro/contas-receber/${editando}`, payload);
      } else {
        await axios.post(`${API}/financeiro/contas-receber`, payload);
      }
      setModal(false);
      carregar();
    } catch (e) {
  ...
  const marcarRecebido = async (id) => {
    await axios.patch(`${API}/financeiro/contas-receber/${id}/receber`);
    carregar();
  };

  const excluir = async (id) => {
    if (!window.confirm('Excluir este lançamento?')) return;
    await axios.delete(`${API}/financeiro/contas-receber/${id}`);
    carregar();
  };
      ...
      <div className="fluxo-tabelas-lado">
        {empresas.map(emp => (
          <div key={emp.id} className="fluxo-empresa-col">
            <div className="fluxo-empresa-titulo">{emp.nome}</div>
            <table className="fluxo-table">
              ...
              <tbody>
                ...
                {contasDaEmpresa(emp.id).map(c => (
                  <tr key={c.id} className={c.status === 'recebido' ? 'fluxo-row-ok' : ''}>
                    <td>{c.descricao}</td>
                    <td>{formatarValor(c.valor)}</td>
                    <td>{formatarData(c.data_vencimento)}</td>
                    <td>
                      <span className={`fluxo-badge ${c.status === 'recebido' ? 'badge-ok' : 'badge-pend'}`}>
                        {c.status === 'recebido' ? 'Recebido' : 'Pendente'}
                      </span>
                    </td>
                    <td className="fluxo-acoes">
                      {c.status === 'pendente' && (
                        <button className="fluxo-btn-acao ok" title="Marcar como recebido" onClick={() => marcarRecebido(c.id)}>✓</button>
                      )}
