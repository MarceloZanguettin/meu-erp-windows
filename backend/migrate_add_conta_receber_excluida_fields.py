"""
Migração aditiva: cria/expande a tabela `contas_receber_excluidas` (modelo
ContaReceberExcluida — campos migrados da tabela DELRECEBER do GENUS), sem
apagar nada.
...
Execute com: python migrate_add_conta_receber_excluida_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ContaReceberExcluida.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ContaReceberExcluida.__table__])
...
with engine.begin() as conn:
    for coluna in ContaReceberExcluida.__table__.columns:
