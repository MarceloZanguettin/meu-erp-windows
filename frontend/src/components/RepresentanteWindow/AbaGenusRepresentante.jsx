...
import { GENUS_REPRESENTANTE_SECOES } from './genusRepresentanteFields.js';

/**
 * Seção "GENUS" do RepresentanteWindow / NovoRepresentanteWindow — exibe/edita
 * todos os campos migrados da tabela REPRESENTANTE do sistema legado GENUS
 * (GENUS_ZANGUETTIN.FDB) que ainda não têm um campo equivalente no
 * formulário principal do ERP.
 */
export default function AbaGenusRepresentante({ form, setField }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      {GENUS_REPRESENTANTE_SECOES.map(secao => (
        <fieldset
          key={secao.titulo}
