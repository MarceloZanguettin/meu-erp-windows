"""
Migração aditiva: cria/expande a tabela `cotacao_produtos` (modelo
CotacaoProduto — campos migrados da tabela COTACAOPRODUTO do GENUS), sem
apagar nada.
...
Execute com: python migrate_add_cotacao_produto_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = CotacaoProduto.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[CotacaoProduto.__table__])
...
with engine.begin() as conn:
    for coluna in CotacaoProduto.__table__.columns:
