"""
Migração aditiva: cria/expande a tabela `entradas_frete` (modelo EntradaFrete —
campos migrados da tabela ENTRADAFRETE do GENUS), sem apagar nada.
...
Execute com: python migrate_add_entrada_frete_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = EntradaFrete.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[EntradaFrete.__table__])
...
with engine.begin() as conn:
    for coluna in EntradaFrete.__table__.columns:
