"""
Migração aditiva: cria/expande a tabela `itens_entrada` (modelo ItemEntrada —
campos migrados da tabela ENTLAN do GENUS), sem apagar nada.
...
Execute com: python migrate_add_item_entrada_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ItemEntrada.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ItemEntrada.__table__])
...
with engine.begin() as conn:
    for coluna in ItemEntrada.__table__.columns:
