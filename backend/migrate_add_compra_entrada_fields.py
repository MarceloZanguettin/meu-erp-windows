"""
Migração aditiva: cria/expande a tabela `compras_entrada` (modelo CompraEntrada —
campos migrados da tabela COMPRAENTRADA do GENUS), sem apagar nada.
...
Execute com: python migrate_add_compra_entrada_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = CompraEntrada.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[CompraEntrada.__table__])
...
with engine.begin() as conn:
    for coluna in CompraEntrada.__table__.columns:
