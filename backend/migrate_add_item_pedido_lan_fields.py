"""
Migração aditiva: cria/expande a tabela `itens_pedido_lan` (modelo
ItemPedidoLan — campos migrados da tabela PEDIDOLAN do GENUS), sem apagar
nada.
...
Execute com: python migrate_add_item_pedido_lan_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ItemPedidoLan.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ItemPedidoLan.__table__])
...
with engine.begin() as conn:
    for coluna in ItemPedidoLan.__table__.columns:
