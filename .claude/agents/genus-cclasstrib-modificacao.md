---
name: genus-cclasstrib-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `CCLASSTRIB` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela CCLASSTRIB", "adapta o backend pra reconhecer os campos de CCLASSTRIB", "migra a estrutura de CCLASSTRIB do GENUS".
...
`CCLASSTRIB`, following the exact precedent already established for
...
## Fields to recognize (GENUS table `CCLASSTRIB`)
...
| PREDIBS | Float |
| PREDCBS | Float |
| IND_REDUTORBC | String(1) |
| IND_GTRIBREGULAR | String(1) |
| DINIVIG | DateTime |
| DFIMVIG | DateTime |
| DATUALIZACAO | DateTime |
| IND_MONOFASICO | String(1) |
| LC | String(30) |
| IND_GCREDPRESOPER | Integer |
| IND_GMONOPADRAO | Integer |
| IND_GMONORETEN | Integer |
| IND_GMONORET | Integer |
| IND_GMONODIF | Integer |
| IND_GESTORNOCRED | Integer |
| IND_NFEABI | Integer |
| IND_NFE | Integer |
| IND_NFCE | Integer |
| IND_CTE | Integer |
