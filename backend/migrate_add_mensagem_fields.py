"""
Migração aditiva: cria/expande a tabela `mensagens` com as colunas do modelo
Mensagem (campos migrados da tabela MENSAGEM do GENUS), sem apagar nada.
...
1. Se a tabela `mensagens` ainda não existir, cria SOMENTE essa tabela via
...
Execute com: python migrate_add_mensagem_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[Mensagem.__table__])
...
with engine.begin() as conn:
    for coluna in Mensagem.__table__.columns:
...
print("Nenhum dado existente em mensagens (ou em qualquer outra tabela) foi apagado ou alterado.")
