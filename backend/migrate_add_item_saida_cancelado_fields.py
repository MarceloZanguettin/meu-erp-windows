"""
Migração aditiva: cria/expande a tabela `itens_saida_cancelados` (modelo
ItemSaidaCancelado — campos migrados da tabela SAILAN_CANCELADA do GENUS),
sem apagar nada.
...
Execute com: python migrate_add_item_saida_cancelado_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ItemSaidaCancelado.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ItemSaidaCancelado.__table__])
...
with engine.begin() as conn:
    for coluna in ItemSaidaCancelado.__table__.columns:
