"""
Migração aditiva: cria/expande a tabela `requisicao_materia_etapas` (modelo
RequisicaoMateriaEtapas — campos migrados da tabela REQUISICAOMATERIAETAPAS
do GENUS), sem apagar nada.
...
Execute com: python migrate_add_requisicao_materia_etapas_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = RequisicaoMateriaEtapas.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[RequisicaoMateriaEtapas.__table__])
...
with engine.begin() as conn:
    for coluna in RequisicaoMateriaEtapas.__table__.columns:
