---
name: genus-receber-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `RECEBER` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela RECEBER", "adapta o backend pra reconhecer os campos de RECEBER", "migra a estrutura de RECEBER do GENUS".
...
`RECEBER`, following the exact precedent already established for
...
Provavelmente corresponde ao model ContaReceber já existente.

## Fields to recognize (GENUS table `RECEBER`)
...
| DTPAGO | DateTime |
| VALORPAGO | Float |
| CODHISTORICO | String(12) |
| CODCONTAS | Integer |
| DTDIGITACAO | DateTime |
| OBS | String(500) |
| TIPODOC | String(1) |
| DOC | Integer |
| CODEMPRESAREC | Integer |
| IMPBOLETO | String(1) |
| CODMOVTO | Integer |
| NOSSONUMERO | String(20) |
| CODCARTEIRA | Integer |
| CODFATURA | Integer |
| COMISSAO | Float |
| PROCESSAMENTO | DateTime |
| SCPCENVIADO | DateTime |
| SCPCRETIRADO | DateTime |
| CARTACOBRANCA | DateTime |
| CARTASCPC | DateTime |
...
| PROTOCOLOPROTESTO | String(30) |
| OBSPROTESTO | String(80) |
| OBSRETIRAPROTESTO | String(80) |
| VALORCARTORIO | Float |
| PISCOFINS | Float |
| ISS | Float |
| OCORRENCIA | String(80) |
| VALORMULTA | Float |
| VALORMORA | Float |
| VALORDESCONTO | Float |
| NUMTRANSACAO | String(15) |
| VALORFINANCEIRO | Float |
| OBSBOLETO | String(160) |
| FUNCIONARIOBAIXA | Integer |
| CODFRETE | Integer |
| CODREPRESENTANTE | Integer |
| CODLOCACAO | Integer |
| CODEMPRESASAIDA | Integer |
| CODFIXO | Integer |
| BANCOREMESSA | String(20) |
| VALORDEPOSITO | Float |
| COD_ANTIGO_RECEBER | Integer |
