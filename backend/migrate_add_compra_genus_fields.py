"""
Migração aditiva: cria/expande a tabela `compras_genus` (modelo CompraGenus —
campos migrados da tabela COMPRAS do GENUS, o cabeçalho de solicitação/pedido
de compra), sem apagar nada.
...
Execute com: python migrate_add_compra_genus_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = CompraGenus.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[CompraGenus.__table__])
...
with engine.begin() as conn:
    for coluna in CompraGenus.__table__.columns:
