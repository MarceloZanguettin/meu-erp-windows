"""
Migração aditiva: cria/expande a tabela `saidas_canceladas` (modelo
SaidaCancelada — campos migrados da tabela SAIDA_CANCELADA do GENUS), sem
apagar nada.
...
Execute com: python migrate_add_saida_cancelada_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = SaidaCancelada.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[SaidaCancelada.__table__])
...
with engine.begin() as conn:
    for coluna in SaidaCancelada.__table__.columns:
