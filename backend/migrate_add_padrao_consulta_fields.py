"""
Migração aditiva: cria/expande a tabela `padroes_consulta` com as colunas do
modelo PadraoConsulta (campos migrados da tabela PADRAOCONSULTA do GENUS),
sem apagar nada.
...
1. Se a tabela `padroes_consulta` ainda não existir, cria SOMENTE essa
...
Execute com: python migrate_add_padrao_consulta_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = PadraoConsulta.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[PadraoConsulta.__table__])
...
with engine.begin() as conn:
    for coluna in PadraoConsulta.__table__.columns:
...
print("Nenhum dado existente em padroes_consulta (ou em qualquer outra tabela) foi apagado ou alterado.")
