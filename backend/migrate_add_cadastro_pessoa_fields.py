"""
Migração aditiva: cria/atualiza a tabela `cadastros_pessoa` com as colunas do
modelo CadastroPessoa (campos migrados da tabela CADASTRO do GENUS), sem
apagar nada.
...
Nota: a coluna `codigo` é criada sem constraint UNIQUE (para não falhar em
bancos com dados divergentes); a checagem de duplicidade na criação de
cadastros já é feita pela aplicação em controllers/cadastro_pessoa_controller.py.

Execute com: python migrate_add_cadastro_pessoa_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = CadastroPessoa.__tablename__
...
with engine.begin() as conn:
    for coluna in CadastroPessoa.__table__.columns:
...
print("Nenhum dado existente em cadastros_pessoa (ou em qualquer outra tabela) foi apagado ou alterado.")
