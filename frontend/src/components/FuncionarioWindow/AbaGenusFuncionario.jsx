...
import { GENUS_FUNCIONARIO_SECOES } from './genusFuncionarioFields.js';

/**
 * Seção "GENUS" do FuncionarioWindow / NovoFuncionarioWindow — exibe/edita
 * todos os campos migrados da tabela FUNCIONARIO do sistema legado GENUS
 * (GENUS_ZANGUETTIN.FDB) que ainda não têm um campo equivalente no
 * formulário principal do ERP.
 */
export default function AbaGenusFuncionario({ form, setField }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      {GENUS_FUNCIONARIO_SECOES.map(secao => (
        <fieldset
          key={secao.titulo}
