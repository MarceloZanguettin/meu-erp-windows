"""
Migração aditiva: cria/expande a tabela `clientes_empresa` (modelo
ClienteEmpresa — campos migrados da tabela CLIENTEEMPRESA do GENUS, o
vínculo entre um cliente e a(s) empresa(s)/filial(is) em que ele está
cadastrado), sem apagar nada.
...
Execute com: python migrate_add_cliente_empresa_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ClienteEmpresa.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ClienteEmpresa.__table__])
...
with engine.begin() as conn:
    for coluna in ClienteEmpresa.__table__.columns:
