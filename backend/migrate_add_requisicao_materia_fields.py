"""
Migração aditiva: cria/expande a tabela `requisicoes_materia` (modelo
RequisicaoMateria — campos migrados da tabela REQUISICAOMATERIA do GENUS,
cabeçalho da requisição de material), sem apagar nada.
...
Execute com: python migrate_add_requisicao_materia_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = RequisicaoMateria.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[RequisicaoMateria.__table__])
...
with engine.begin() as conn:
    for coluna in RequisicaoMateria.__table__.columns:
