"""
Migração aditiva: cria/expande a tabela `produto_composicoes` (modelo
ProdutoComposicao — campos migrados da tabela PRODUTOCOMPOSICAO do GENUS),
sem apagar nada.
...
Execute com: python migrate_add_produto_composicao_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ProdutoComposicao.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ProdutoComposicao.__table__])
...
with engine.begin() as conn:
    for coluna in ProdutoComposicao.__table__.columns:
