"""
Migração aditiva: cria/expande a tabela `restricoes` com as colunas do
modelo Restricao (campos migrados da tabela RESTRICAO do GENUS), sem
apagar nada.
...
1. Se a tabela `restricoes` ainda não existir, cria SOMENTE essa tabela via
...
Execute com: python migrate_add_restricao_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = Restricao.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[Restricao.__table__])
...
with engine.begin() as conn:
    for coluna in Restricao.__table__.columns:
...
print("Nenhum dado existente em restricoes (ou em qualquer outra tabela) foi apagado ou alterado.")
