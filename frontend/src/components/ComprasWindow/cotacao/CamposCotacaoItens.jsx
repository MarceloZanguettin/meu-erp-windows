...
import { GRUPOS_CAMPOS_COTACAO_ITENS } from './camposCotacaoItens.js';
...
/**
 * Renderiza todos os campos de GENUS.COTACAOITENS agrupados em fieldsets, a
 * partir da config declarativa em camposCotacaoItens.js. Usado tanto pela
 * janela de edição (CotacaoItensWindow) quanto pela janela de criação
 * (NovaCotacaoItensWindow) — mesmo padrão de
 * ComprasWindow/compra/CamposCompraGenus.jsx (GENUS.COMPRAS).
 */
export default function CamposCotacaoItens({ form, setForm }) {
  const setField = (campo, valor) => setForm({ ...form, [campo]: valor });

  return (
    <>
      {GRUPOS_CAMPOS_COTACAO_ITENS.map(grupo => (
