"""
Migração aditiva: cria/expande a tabela `produto_processos` (modelo
ProdutoProcesso — campos migrados da tabela PRODUTOPROCESSO do GENUS), sem
apagar nada.
...
Execute com: python migrate_add_produto_processo_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ProdutoProcesso.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ProdutoProcesso.__table__])
...
with engine.begin() as conn:
    for coluna in ProdutoProcesso.__table__.columns:
