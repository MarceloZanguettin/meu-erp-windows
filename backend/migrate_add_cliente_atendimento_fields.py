"""
Migração aditiva: cria/expande a tabela `clientes_atendimento` com as colunas
do modelo ClienteAtendimento (campos migrados da tabela CLIENTEATENDIMENTO do
GENUS), sem apagar nada.
...
1. Se a tabela `clientes_atendimento` ainda não existir, cria SOMENTE essa
...
Execute com: python migrate_add_cliente_atendimento_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ClienteAtendimento.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ClienteAtendimento.__table__])
...
with engine.begin() as conn:
    for coluna in ClienteAtendimento.__table__.columns:
...
print("Nenhum dado existente em clientes_atendimento (ou em qualquer outra tabela) foi apagado ou alterado.")
