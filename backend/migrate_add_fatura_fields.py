"""
Migração aditiva: cria/expande a tabela `faturas` com as colunas do modelo
Fatura (campos migrados da tabela FATURA do GENUS), sem apagar nada.
...
Execute com: python migrate_add_fatura_fields.py (dentro da pasta backend,
com o venv ativo)
"""
import sys
import os
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[Fatura.__table__])
...
with engine.begin() as conn:
    for coluna in Fatura.__table__.columns:
...
print("Nenhum dado existente em faturas (ou em qualquer outra tabela) foi apagado ou alterado.")
