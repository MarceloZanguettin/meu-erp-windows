"""
Migração aditiva: cria/expande a tabela `paises` com as colunas do modelo
Pais (campos migrados da tabela PAIS do GENUS), sem apagar nada.
...
1. Se a tabela `paises` ainda não existir, cria SOMENTE essa tabela via
...
Execute com: python migrate_add_pais_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[Pais.__table__])
...
with engine.begin() as conn:
    for coluna in Pais.__table__.columns:
...
print("Nenhum dado existente em paises (ou em qualquer outra tabela) foi apagado ou alterado.")
