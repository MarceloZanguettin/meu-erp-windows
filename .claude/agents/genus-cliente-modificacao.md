---
name: genus-cliente-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `CLIENTE` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela CLIENTE", "adapta o backend pra reconhecer os campos de CLIENTE", "migra a estrutura de CLIENTE do GENUS".
...
`CLIENTE`, following the exact precedent already established for
...
Provavelmente corresponde ao model ClienteCompleto (backend/models/tabelas.py) já existente — precisa fazer join com CADASTRO (ver precedente do Produto/CLIENTE já mapeado).

## Fields to recognize (GENUS table `CLIENTE`)
...
| RENDA | Float |
| TRABALHO | String(30) |
| FONE_TRABALHO | String(15) |
| DTADMISSAO | DateTime |
| CONTATO_TRABALHO | String(15) |
| CODNATURALIDADE | Integer |
| ORGAOEXP | String(5) |
| DTEXPEDICAO | DateTime |
| COB_ENDERECO | String(50) |
| COB_BAIRRO | String(23) |
| COB_CEP | String(10) |
| COB_CODCIDADE | Integer |
| CNAE | String(10) |
...
| CARGAMEDIATRIB | Float |
| VALOR_KMRODADO | Float |
| ACRESCIMO | Float |
| CODALTERNATIVO | Integer |
| CODTIPOVENDA | Integer |
| OPERADORA | String(1) |
| CODTABELAPRECO | Integer |
| PRODRURAL | String(1) |
| DATA_IMP_LGPD | DateTime |
| DATA_DEV_LGPD | DateTime |
| HORA_IMP_LGPD | String(8) |
| HORA_DEV_LGPD | String(8) |
| CODFUNCIONARIO_LGPD | Integer |
| REDUZIRICMSBASECALCULOPISCOFINS | String(1) |
| DIASRECORRENCIA | Integer |
| CALCULARDIFAL | String(1) |
| NAODESTACARICMS | String(1) |
