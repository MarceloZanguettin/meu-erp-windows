---
name: genus-ccredpres-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `CCREDPRES` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela CCREDPRES", "adapta o backend pra reconhecer os campos de CCREDPRES", "migra a estrutura de CCREDPRES do GENUS".
...
`CCREDPRES`, following the exact precedent already established for
...
## Fields to recognize (GENUS table `CCREDPRES`)
...
| LC | Text |
| APROPRIANF | String(1) |
| APROPRIAEVENTO | String(1) |
| INDDEDUZCREDPRES | String(1) |
| INDCREDPRESCBS | String(1) |
| INDCREDPRESIBS | String(1) |
| ALIQUOTACBS | String(5) |
| ALIQUOTAIBS | String(5) |
| CCLASSNOTA | String(40) |
| DINIVIG | DateTime |
| DFIMVIG | DateTime |
| PALIQCREDPRESCBS | String(255) |
| DINIVIGCBS | DateTime |
| DFIMVIGCBS | DateTime |
| DINIVIGIBS | DateTime |
| DFIMVIGIBS | DateTime |
| PALIQCREDPRESIBS | String(255) |
