"""
Migração aditiva: cria/expande a tabela `pedidos_nota` (modelo PedidoNota —
campos migrados da tabela PEDIDONOTA do GENUS, o vínculo entre um pedido de
venda e a(s) nota(s) fiscal(is) de saída geradas a partir dele), sem apagar
nada.
...
Execute com: python migrate_add_pedido_nota_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = PedidoNota.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[PedidoNota.__table__])
...
with engine.begin() as conn:
    for coluna in PedidoNota.__table__.columns:
