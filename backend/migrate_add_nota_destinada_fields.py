"""
Migração aditiva: cria/expande a tabela `notas_destinadas` (modelo
NotaDestinada — campos migrados da tabela NOTASDESTINADAS do GENUS, o
manifesto do destinatário / NF-e recebida de terceiros ainda pendente de
manifestação e/ou lançamento como entrada), sem apagar nada.
...
Execute com: python migrate_add_nota_destinada_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = NotaDestinada.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[NotaDestinada.__table__])
...
with engine.begin() as conn:
    for coluna in NotaDestinada.__table__.columns:
