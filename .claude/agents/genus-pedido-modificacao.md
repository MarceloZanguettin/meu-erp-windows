---
name: genus-pedido-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `PEDIDO` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela PEDIDO", "adapta o backend pra reconhecer os campos de PEDIDO", "migra a estrutura de PEDIDO do GENUS".
...
`PEDIDO`, following the exact precedent already established for
...
Pode corresponder ao model Pedido (simples) ou PedidoVenda (mais completo) já existentes — avaliar qual é o alvo certo antes de expandir.

## Fields to recognize (GENUS table `PEDIDO`)
...
| TIPO | String(1) |
| CONTATO | String(40) |
| TELEMARKETING | String(1) |
| TIPOPREPEDIDO | String(1) |
| PERCFRETE | Float |
...
| CODMOVTOGRADE | Integer |
| CODAGREGADO | Integer |
| MOTIVOBLOQUEIO | Text |
| CODLIBERACAO | Integer |
| CTE | Integer |
| NUMERONF | Integer |
| VALORFRETE | Float |
| QTDEKG | Float |
...
| TOTALNF | Float |
| VALORKG | Float |
| TIPOTRANSPORTE | String(1) |
| CODAPROVACAO | Integer |
| DTAPROVACAO | DateTime |
| DTLIBERACAO | DateTime |
| PRODUCAOETAPAS | String(1) |
| PERCDESCONTO | Float |
| OBSINTERNA | Text |
| LIBERADOPARAPRODUCAO | String(1) |
| ORCAMENTONEGADO | String(1) |
| MOTIVOORCAMENTONEGADO | Text |
| CODFUNCORCAMENTONEGADO | Integer |
| LIBERADO | String(1) |
| DESCONTOINTERNO | Float |
| FRETEINTERNO | Float |
| FATURADO | String(1) |
| ANTESBLOQUEADO | String(1) |
| CODEMPRESASAIDAPROD | Integer |
| CODIGOSAIDAPROD | Integer |
| DOCSAIDAPROD | String(20) |
