---
name: genus-entlan-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `ENTLAN` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela ENTLAN", "adapta o backend pra reconhecer os campos de ENTLAN", "migra a estrutura de ENTLAN do GENUS".
...
`ENTLAN`, following the exact precedent already established for
...
## Fields to recognize (GENUS table `ENTLAN`)
...
| RETIRAR | String(1) |
| CODCFOP | String(5) |
| TIPOIMPOSTO | String(1) |
| IVA | Float |
| LOTEPRODUTO | String(15) |
| ICMSPERCENTUALST | Float |
| ICMSREDUCAO | Float |
| ICMSREDUCAOST | Float |
| ICMSVALOR | Float |
| ICMSBASECALCULO | Float |
| ICMSSUBSTTRIBUTARIA | Float |
| ICMSBASESUBSTTRIBUTARIA | Float |
| ICMSISENTO | Float |
| ICMSOUTRAS | Float |
| IPIVALOR | Float |
...
| CODSAIDAVINCULADA | Integer |
| CODEMPRESASAIDAVINCULADA | Integer |
| DOCSAIDAVINCULADA | Integer |
| TOTAL | Float |
| QTDE | Float |
| QTDEESTQ | Float |
| CREDITOFORNECEDOR | Float |
