"""
Migração aditiva: cria/expande a tabela `requisicoes_produto` (modelo
RequisicaoProduto — campos migrados da tabela REQUISICAOPRODUTO do GENUS,
item da requisição de material, elo do meio entre REQUISICAOMATERIA
/RequisicaoMateria e REQUISICAOMATERIAETAPAS/RequisicaoMateriaEtapas), sem
apagar nada.
...
Execute com: python migrate_add_requisicao_produto_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = RequisicaoProduto.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[RequisicaoProduto.__table__])
...
with engine.begin() as conn:
    for coluna in RequisicaoProduto.__table__.columns:
