---
name: genus-partidadobrada-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `PARTIDADOBRADA` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela PARTIDADOBRADA", "adapta o backend pra reconhecer os campos de PARTIDADOBRADA", "migra a estrutura de PARTIDADOBRADA do GENUS".
...
`PARTIDADOBRADA`, following the exact precedent already established for
...
## Fields to recognize (GENUS table `PARTIDADOBRADA`)

| Coluna GENUS | Tipo sugerido (SQLAlchemy/Postgres) |
|---|---|
| CODIGO | Integer |
| CODEMPRESADEB | Integer |
| CODEMPRESACRED | Integer |
| DOCDEB | String(15) |
| DOCCRED | String(15) |
| FORMAPAGTODEB | String(1) |
| FORMAPAGTOCRED | String(1) |
| CODCONTADEB | Integer |
| CODCONTACRED | Integer |
| CODHISTDEB | String(12) |
| CODHISTCRED | String(12) |
| OBSDEB | String(70) |
| OBSCRED | String(70) |
| DATA | DateTime |
| HORA | String(8) |
