"""
Migração aditiva: cria/expande a tabela `cargos` com as colunas do modelo
Cargo (campos migrados da tabela CARGO do GENUS), sem apagar nada.
...
1. Se a tabela `cargos` ainda não existir, cria SOMENTE essa tabela via
...
Execute com: python migrate_add_cargo_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
with engine.begin() as conn:
    for coluna in Cargo.__table__.columns:
...
print("Nenhum dado existente em cargos (ou em qualquer outra tabela) foi apagado ou alterado.")
