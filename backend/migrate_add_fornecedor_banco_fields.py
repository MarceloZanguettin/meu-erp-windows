"""
Migração aditiva: cria/expande a tabela `fornecedores_banco` com as
colunas do modelo FornecedorBanco (campos migrados da tabela
FORNECEDORBANCO do GENUS), sem apagar nada.
...
1. Se a tabela `fornecedores_banco` ainda não existir, cria SOMENTE essa
...
Execute com: python migrate_add_fornecedor_banco_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = FornecedorBanco.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[FornecedorBanco.__table__])
...
with engine.begin() as conn:
    for coluna in FornecedorBanco.__table__.columns:
...
print("Nenhum dado existente em fornecedores_banco (ou em qualquer outra tabela) foi apagado ou alterado.")
