...
import { GRUPOS_CAMPOS_SAIDA } from './camposSaida.js';
...
/**
 * Renderiza todos os campos de GENUS.SAIDA agrupados em fieldsets,
 * a partir da config declarativa em camposSaida.js. Usado tanto pela janela
 * de edição (SaidaWindow) quanto pela janela de criação (NovaSaidaWindow).
 */
export default function CamposSaida({ form, setForm }) {
  const setField = (campo, valor) => setForm({ ...form, [campo]: valor });

  return (
    <>
      {GRUPOS_CAMPOS_SAIDA.map(grupo => (
