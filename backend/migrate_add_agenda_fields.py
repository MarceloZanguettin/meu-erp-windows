"""
Migração aditiva: cria/expande a tabela `agendas` com as colunas do modelo
Agenda (campos migrados da tabela AGENDA do GENUS), sem apagar nada.
...
1. Se a tabela `agendas` ainda não existir, cria SOMENTE essa tabela via
...
Execute com: python migrate_add_agenda_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[Agenda.__table__])
...
with engine.begin() as conn:
    for coluna in Agenda.__table__.columns:
...
print("Nenhum dado existente em agendas (ou em qualquer outra tabela) foi apagado ou alterado.")
