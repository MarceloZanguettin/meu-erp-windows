"""
Migração aditiva: cria/expande a tabela `faturas_nota` (modelo FaturaNota —
campos migrados da tabela FATURANOTA do GENUS, o vínculo entre uma fatura
(agrupamento de título(s) a receber) e a(s) nota(s) fiscal(is) de saída que a
compõem), sem apagar nada.
...
Execute com: python migrate_add_fatura_nota_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = FaturaNota.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[FaturaNota.__table__])
...
with engine.begin() as conn:
    for coluna in FaturaNota.__table__.columns:
