---
name: genus-manifesto-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `MANIFESTO` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela MANIFESTO", "adapta o backend pra reconhecer os campos de MANIFESTO", "migra a estrutura de MANIFESTO do GENUS".
...
`MANIFESTO`, following the exact precedent already established for
...
## Fields to recognize (GENUS table `MANIFESTO`)
...
| CODCIDADE | Integer |
| CODVEICULO | Integer |
| INFADICIONAISFISCO | Text |
| INFADICIONAISCONTRIBUINTE | Text |
| HORA | String(8) |
| CHAVE | String(70) |
| CANCELADO | String(1) |
| PAGAMENTO_RESPONSAVEL | String(150) |
| PAGAMENTO_IDESTRANGEIRO | String(20) |
| PAGAMENTO_CNPJRESPONSAVEL | String(20) |
| PAGAMENTO_VALORCONTRATO | Float |
| PAGAMENTO_ALTODESEMPENHO | String(1) |
| PAGAMENTO_TIPOPAGTO | String(1) |
| PAGAMENTO_VALORADIANTAMENTO | Float |
| PAGAMENTO_ANTECIPARADIANTAMENTO | String(1) |
| PAGAMENTO_ANTECADIANTAMENTOTP | String(1) |
| PAGAMENTO_AGENCIA | String(20) |
| PAGAMENTO_BANCO | String(20) |
| PAGAMENTO_CNPJIPEF | String(20) |
| PAGAMENTO_CHAVEPIX | String(150) |
