"""
Migração aditiva: adiciona à tabela `formas_pagamento` as colunas novas
migradas da tabela CONDPAGTO do GENUS (GENUS_ZANGUETTIN.FDB), sem apagar nada.

FormaPagamento já existia neste ERP como "condição de pagamento" e é a
mesma entidade de CONDPAGTO no GENUS — por isso os campos legados foram
adicionados ao model existente (backend/models/tabelas.py) em vez de criar
uma tabela nova.
...
Execute com: python migrate_add_condpagto_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
from models.tabelas import FormaPagamento

inspector = inspect(engine)
tabela = FormaPagamento.__tablename__
...
with engine.begin() as conn:
    for coluna in FormaPagamento.__table__.columns:
...
print("Nenhum dado existente em formas_pagamento (ou em qualquer outra tabela) foi apagado ou alterado.")
