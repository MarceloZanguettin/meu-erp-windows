"""
Migração aditiva: cria/expande a tabela `cotacao_itens` (modelo CotacaoItens —
campos migrados da tabela COTACAOITENS do GENUS), sem apagar nada.
...
Execute com: python migrate_add_cotacao_itens_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = CotacaoItens.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[CotacaoItens.__table__])
...
with engine.begin() as conn:
    for coluna in CotacaoItens.__table__.columns:
