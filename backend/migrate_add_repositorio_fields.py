"""
Migração aditiva: cria/expande a tabela `repositorios` com as colunas do
modelo Repositorio (campos migrados da tabela REPOSITORIO do GENUS), sem
apagar nada.
...
1. Se a tabela `repositorios` ainda não existir, cria SOMENTE essa tabela via
   `Base.metadata.create_all(bind=engine, tables=[...])` — o mesmo
   mecanismo não-destrutivo que `main.py` já usa no startup
...
Execute com: python migrate_add_repositorio_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = Repositorio.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[Repositorio.__table__])
...
with engine.begin() as conn:
    for coluna in Repositorio.__table__.columns:
...
print("Nenhum dado existente em repositorios (ou em qualquer outra tabela) foi apagado ou alterado.")
