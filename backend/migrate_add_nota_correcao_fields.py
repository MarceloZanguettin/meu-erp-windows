"""
Migração aditiva: cria/expande a tabela `notas_correcao` (modelo NotaCorrecao —
campos migrados da tabela NOTACORRECAO do GENUS, a Carta de Correção
Eletrônica (CC-e) vinculada a uma saída/nota fiscal de venda), sem apagar
nada.
...
Execute com: python migrate_add_nota_correcao_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = NotaCorrecao.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[NotaCorrecao.__table__])
...
with engine.begin() as conn:
    for coluna in NotaCorrecao.__table__.columns:
