"""
Migração aditiva: cria/expande a tabela `ivas` com as colunas do modelo
Iva (campos migrados da tabela IVA do GENUS), sem apagar nada.
...
1. Se a tabela `ivas` ainda não existir, cria SOMENTE essa tabela via
...
Execute com: python migrate_add_iva_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[Iva.__table__])
...
with engine.begin() as conn:
    for coluna in Iva.__table__.columns:
...
print("Nenhum dado existente em ivas (ou em qualquer outra tabela) foi apagado ou alterado.")
