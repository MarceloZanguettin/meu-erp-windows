...
import { GENUS_FORNECEDOR_SECOES } from './genusFornecedorFields.js';

/**
 * Aba "GENUS" do FornecedorWindow / NovoFornecedorWindow — exibe/edita todos
 * os campos migrados da tabela FORNECEDOR do sistema legado GENUS
 * (GENUS_ZANGUETTIN.FDB) que ainda não têm um campo equivalente no
 * formulário principal do ERP.
 */
export default function AbaGenusFornecedor({ form, setField }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
      {GENUS_FORNECEDOR_SECOES.map(secao => (
        <fieldset
          key={secao.titulo}
