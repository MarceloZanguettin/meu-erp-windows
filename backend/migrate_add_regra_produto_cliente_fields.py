"""
Migração aditiva: cria/expande a tabela `regras_produto_cliente` (modelo
RegraProdutoCliente — campos migrados da tabela REGRASPRODCLI do GENUS), sem
apagar nada.
...
Execute com: python migrate_add_regra_produto_cliente_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = RegraProdutoCliente.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[RegraProdutoCliente.__table__])
...
with engine.begin() as conn:
    for coluna in RegraProdutoCliente.__table__.columns:
