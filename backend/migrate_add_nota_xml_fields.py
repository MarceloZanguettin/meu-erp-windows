"""
Migração aditiva: cria/expande a tabela `notas_xml` (modelo NotaXml — campos
migrados da tabela NOTAXML do GENUS, o XML da NF-e emitida vinculado a uma
saída/nota fiscal de venda), sem apagar nada.
...
Execute com: python migrate_add_nota_xml_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[NotaXml.__table__])
...
with engine.begin() as conn:
    for coluna in NotaXml.__table__.columns:
