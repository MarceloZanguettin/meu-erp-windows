---
name: genus-centrocusto-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `CENTROCUSTO` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela CENTROCUSTO", "adapta o backend pra reconhecer os campos de CENTROCUSTO", "migra a estrutura de CENTROCUSTO do GENUS".
...
`CENTROCUSTO`, following the exact precedent already established for
...
Provavelmente corresponde ao model CentroCusto já existente.

## Fields to recognize (GENUS table `CENTROCUSTO`)
...
| CUSTOFIXO | Float |
| MARGEMLUCRO | Float |
| DIFERENCASUBST | Float |
| DIFERENCAICMS | Float |
| COMISSAO | Float |
| AVISTA | Float |
| COMISSAOAVISTA | Float |
| PERCENTUALAVISTA | Float |
| PERTENCEEMPRESA | String(1) |
| PRECOMINIMO | Float |
| IPI_ENTRADA | Float |
| TECLABALANCA | Integer |
| TIPOBALANCA | String(1) |
| VALIDADE | Integer |
| CODBALANCA | Integer |
| PIS_CST | String(3) |
| COFINS_CST | String(3) |
...
| PIS_REAIS | Float |
| COFINS_REAIS | Float |
| PERCENTUALAPRAZO | Float |
| PERCENTUALMINIMO | Float |
| AQUISICAO | DateTime |
| NOTAPATRIMONIO | Integer |
| VALORPATRIMONIO | Float |
| DATAGARANTIA | DateTime |
| DATADEPRECIACAO | DateTime |
...
| CAPACIDADE | Float |
| TROCAOLEOKM | Integer |
| DATATROCAOLEO | DateTime |
| CODPATRIMONIO | Integer |
| ULTIMOCUSTO | Float |
| MAODEOBRA | Float |
| CUSTOMATERIA | Float |
| LOCALIZACAOPRODUTO | String(50) |
| DATAALTERACAO | DateTime |
| IPICST_ENTRADA | String(3) |
| IPICST_SAIDA | String(3) |
| UNITARIOCOMPRA | Float |
| FORNECEDORCOMPRA | Integer |
| CUSTOMEDIO | Float |
| COFINS_CST_ENTRADA | String(3) |
| COFINS_ALIQUOTA_ENTRADA | Float |
| COFINS_REAIS_ENTRADA | Float |
| PIS_CST_ENTRADA | String(3) |
| PIS_ALIQUOTA_ENTRADA | Float |
| PIS_REAIS_ENTRADA | Float |
