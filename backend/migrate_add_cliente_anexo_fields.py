"""
Migração aditiva: cria/expande a tabela `clientes_anexo` com as colunas
do modelo ClienteAnexo (campos migrados da tabela CLIENTEANEXO do GENUS),
sem apagar nada.
...
1. Se a tabela `clientes_anexo` ainda não existir, cria SOMENTE essa
...
Execute com: python migrate_add_cliente_anexo_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ClienteAnexo.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ClienteAnexo.__table__])
...
with engine.begin() as conn:
    for coluna in ClienteAnexo.__table__.columns:
...
print("Nenhum dado existente em clientes_anexo (ou em qualquer outra tabela) foi apagado ou alterado.")
