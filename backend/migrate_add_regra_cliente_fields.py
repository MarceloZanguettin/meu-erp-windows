"""
Migração aditiva: cria/expande a tabela `regras_cliente` (modelo
RegraCliente — campos migrados da tabela REGRASCLIENTE do GENUS), sem
apagar nada.
...
Execute com: python migrate_add_regra_cliente_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = RegraCliente.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[RegraCliente.__table__])
...
with engine.begin() as conn:
    for coluna in RegraCliente.__table__.columns:
