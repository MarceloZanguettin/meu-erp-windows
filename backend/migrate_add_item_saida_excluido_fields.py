"""
Migração aditiva: cria/expande a tabela `itens_saida_excluidos` (modelo
ItemSaidaExcluido — campos migrados da tabela DELSAILAN do GENUS), sem
apagar nada.
...
Execute com: python migrate_add_item_saida_excluido_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ItemSaidaExcluido.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ItemSaidaExcluido.__table__])
...
with engine.begin() as conn:
    for coluna in ItemSaidaExcluido.__table__.columns:
