"""
Migração aditiva: cria/expande a tabela `notas_xml_entrada` (modelo
NotaXmlEntrada — campos migrados da tabela NOTAXMLENTRADA do GENUS, o XML da
NF-e recebida vinculado a uma entrada/nota fiscal de compra), sem apagar nada.
...
Execute com: python migrate_add_nota_xml_entrada_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = NotaXmlEntrada.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[NotaXmlEntrada.__table__])
...
with engine.begin() as conn:
    for coluna in NotaXmlEntrada.__table__.columns:
