"""
Migração aditiva: cria/expande a tabela `faturas_nota_pagar` (modelo
FaturaNotaPagar — campos migrados da tabela FATURANOTAPAGAR do GENUS, o
vínculo entre uma fatura a pagar (agrupamento de título(s) a pagar) e a(s)
nota(s) fiscal(is) de compra/entrada que a compõem), sem apagar nada.
...
Execute com: python migrate_add_fatura_nota_pagar_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = FaturaNotaPagar.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[FaturaNotaPagar.__table__])
...
with engine.begin() as conn:
    for coluna in FaturaNotaPagar.__table__.columns:
