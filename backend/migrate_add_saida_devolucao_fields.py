"""
Migração aditiva: cria/expande a tabela `saidas_devolucao` (modelo
SaidaDevolucao — campos migrados da tabela SAIDADEVOLUCAO do GENUS, a
devolução de mercadoria vinculada a uma saída/nota fiscal de venda), sem
apagar nada.
...
Execute com: python migrate_add_saida_devolucao_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = SaidaDevolucao.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[SaidaDevolucao.__table__])
...
with engine.begin() as conn:
    for coluna in SaidaDevolucao.__table__.columns:
