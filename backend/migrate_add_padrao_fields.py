"""
Migração aditiva: cria/expande a tabela `padroes` com as colunas do modelo
Padrao (campos migrados da tabela PADRAO do GENUS), sem apagar nada.
...
1. Se a tabela `padroes` ainda não existir, cria SOMENTE essa tabela via
...
Execute com: python migrate_add_padrao_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[Padrao.__table__])
...
with engine.begin() as conn:
    for coluna in Padrao.__table__.columns:
...
print("Nenhum dado existente em padroes (ou em qualquer outra tabela) foi apagado ou alterado.")
