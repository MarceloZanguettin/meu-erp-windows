...
import { GRUPOS_CAMPOS_ENTRADA } from './camposEntrada.js';
...
/**
 * Renderiza todos os campos de GENUS.ENTRADA agrupados em fieldsets,
 * a partir da config declarativa em camposEntrada.js. Usado tanto pela
 * janela de edição (EntradaWindow) quanto pela janela de criação
 * (NovaEntradaWindow) — mesmo padrão de VendasWindow/saida/CamposSaida.jsx.
 */
export default function CamposEntrada({ form, setForm }) {
  const setField = (campo, valor) => setForm({ ...form, [campo]: valor });

  return (
    <>
      {GRUPOS_CAMPOS_ENTRADA.map(grupo => (
