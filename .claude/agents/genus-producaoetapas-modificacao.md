---
name: genus-producaoetapas-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `PRODUCAOETAPAS` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela PRODUCAOETAPAS", "adapta o backend pra reconhecer os campos de PRODUCAOETAPAS", "migra a estrutura de PRODUCAOETAPAS do GENUS".
...
`PRODUCAOETAPAS`, following the exact precedent already established for
...
## Fields to recognize (GENUS table `PRODUCAOETAPAS`)
...
| HORAPRODUCAO | String(8) |
| DATAPREVISAO | DateTime |
| HORAPREVISAO | String(10) |
| FECHADO | DateTime |
| HORAFECHADO | String(8) |
| CODFUNCIONARIO | Integer |
| OBS | Text |
| CODALTERACAO | Integer |
| HORAALTERACAO | String(8) |
| DATAALTERACAO | DateTime |
| IMPCONFECCAO | String(1) |
| IMPSAPATILHAS | String(1) |
| IMPPONTEIRAS | String(1) |
