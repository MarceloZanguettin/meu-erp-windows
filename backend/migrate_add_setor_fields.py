"""
Migração aditiva: cria/expande a tabela `setores` com as colunas do modelo
Setor (campos migrados da tabela SETOR do GENUS), sem apagar nada.
...
1. Se a tabela `setores` ainda não existir, cria SOMENTE essa tabela via
...
Execute com: python migrate_add_setor_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[Setor.__table__])
...
with engine.begin() as conn:
    for coluna in Setor.__table__.columns:
...
print("Nenhum dado existente em setores (ou em qualquer outra tabela) foi apagado ou alterado.")
