"""
Migração aditiva: cria/expande a tabela `cidades` com as colunas do modelo
Cidade (campos migrados da tabela CIDADE do GENUS), sem apagar nada.
...
1. Se a tabela `cidades` ainda não existir, cria SOMENTE essa tabela via
...
Execute com: python migrate_add_cidade_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[Cidade.__table__])
...
with engine.begin() as conn:
    for coluna in Cidade.__table__.columns:
...
print("Nenhum dado existente em cidades (ou em qualquer outra tabela) foi apagado ou alterado.")
