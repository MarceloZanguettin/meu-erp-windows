"""
Migração aditiva: cria/expande a tabela `movto_produtos` (modelo
MovtoProduto — campos migrados da tabela MOVTOPRODUTO do GENUS), sem apagar
nada.
...
Execute com: python migrate_add_movto_produto_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = MovtoProduto.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[MovtoProduto.__table__])
...
with engine.begin() as conn:
    for coluna in MovtoProduto.__table__.columns:
