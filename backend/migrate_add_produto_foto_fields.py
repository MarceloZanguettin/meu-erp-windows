"""
Migração aditiva: cria/expande a tabela `produto_fotos` (modelo
ProdutoFoto — campos migrados da tabela PRODUTOFOTO do GENUS), sem apagar
nada.
...
Execute com: python migrate_add_produto_foto_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ProdutoFoto.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ProdutoFoto.__table__])
...
with engine.begin() as conn:
    for coluna in ProdutoFoto.__table__.columns:
