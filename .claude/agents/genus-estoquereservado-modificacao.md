---
name: genus-estoquereservado-modificacao
description: Use this agent when asked to adapt this ERP's backend and frontend to recognize all fields of the GENUS legacy table `ESTOQUERESERVADO` (from GENUS_ZANGUETTIN.FDB, a Firebird database being progressively migrated into this project's PostgreSQL-backed ERP -- see the project's `genus-fdb-migration-audit` context). This agent does the *structural* work only (SQLAlchemy model, Pydantic schema, FastAPI controller, additive Postgres migration, frontend window/fields) -- it does NOT import any row data from GENUS. Examples: "roda o agente de modificacao da tabela ESTOQUERESERVADO", "adapta o backend pra reconhecer os campos de ESTOQUERESERVADO", "migra a estrutura de ESTOQUERESERVADO do GENUS".
...
`ESTOQUERESERVADO`, following the exact precedent already established for
...
## Fields to recognize (GENUS table `ESTOQUERESERVADO`)
...
| DTMOVTO | DateTime |
| FISICO | String(1) |
| CRIADOMANUTENCAO | String(1) |
| CODFUNCIONARIOMANUTENCAO | Integer |
| DTMANUTENCAO | DateTime |
| DTALTERADOMANUTENCAO | DateTime |
| CODFUNCIONARIOALTERAMANUTENCAO | Integer |
| CODSAIDA | Integer |
| CODSAILAN | Integer |
