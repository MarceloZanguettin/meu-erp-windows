"""
Migração aditiva: cria/expande a tabela `produto_referencias` (modelo
ProdutoReferencia — campos migrados da tabela PRODUTOREFERENCIA do GENUS),
sem apagar nada.
...
Execute com: python migrate_add_produto_referencia_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ProdutoReferencia.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ProdutoReferencia.__table__])
...
with engine.begin() as conn:
    for coluna in ProdutoReferencia.__table__.columns:
