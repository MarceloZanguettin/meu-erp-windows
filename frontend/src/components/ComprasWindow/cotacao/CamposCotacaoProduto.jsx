...
import { GRUPOS_CAMPOS_COTACAO_PRODUTO } from './camposCotacaoProduto.js';
...
/**
 * Renderiza todos os campos de GENUS.COTACAOPRODUTO agrupados em fieldsets, a
 * partir da config declarativa em camposCotacaoProduto.js. Usado tanto pela
 * janela de edição (CotacaoProdutoWindow) quanto pela janela de criação
 * (NovaCotacaoProdutoWindow) — mesmo padrão de
 * ComprasWindow/cotacao/CamposCotacaoItens.jsx (GENUS.COTACAOITENS).
 */
export default function CamposCotacaoProduto({ form, setForm }) {
  const setField = (campo, valor) => setForm({ ...form, [campo]: valor });

  return (
    <>
      {GRUPOS_CAMPOS_COTACAO_PRODUTO.map(grupo => (
