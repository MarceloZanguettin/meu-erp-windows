"""
Migração aditiva: cria/expande a tabela `clientes_cnae` com as colunas
do modelo ClienteCnae (campos migrados da tabela CLIENTECNAE do GENUS),
sem apagar nada.
...
1. Se a tabela `clientes_cnae` ainda não existir, cria SOMENTE essa
...
Execute com: python migrate_add_cliente_cnae_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
tabela = ClienteCnae.__tablename__
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[ClienteCnae.__table__])
...
with engine.begin() as conn:
    for coluna in ClienteCnae.__table__.columns:
...
print("Nenhum dado existente em clientes_cnae (ou em qualquer outra tabela) foi apagado ou alterado.")
