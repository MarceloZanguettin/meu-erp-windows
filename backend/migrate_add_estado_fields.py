"""
Migração aditiva: cria/expande a tabela `estados` com as colunas do modelo
Estado (campos migrados da tabela ESTADO do GENUS), sem apagar nada.
...
1. Se a tabela `estados` ainda não existir, cria SOMENTE essa tabela via
...
Execute com: python migrate_add_estado_fields.py (dentro da pasta backend, com o venv ativo)
"""
import sys
import os
...
if tabela not in inspector.get_table_names():
    ...
    Base.metadata.create_all(bind=engine, tables=[Estado.__table__])
...
with engine.begin() as conn:
    for coluna in Estado.__table__.columns:
...
print("Nenhum dado existente em estados (ou em qualquer outra tabela) foi apagado ou alterado.")
